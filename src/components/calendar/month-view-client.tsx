...
function buildMonthDays(
  ...
  for (
    ...
    cursor.isBefore(monthEnd.endOf("week")) ||
    cursor.isSame(monthEnd.endOf("week"), "day");
...
function MobileDayCard({ day, month }: { day: DayCell; month: string }) {
  ...
  return (
    <Link
      ...
      className={`block rounded-[24px] border bg-white p-4 shadow-sm transition hover:shadow-md ${
        inMonth ? "" : "opacity-55"
      } ${styles.dayClass}`}
    >
      ...
      <div className="mb-3 flex items-center gap-2">
        <div className="h-2 flex-1 rounded-full bg-slate-100">
          <div
            ...
            style={{ width: `${Math.min((stats.totalMinutes / 540) * 100, 100)}%` }}
          />
        </div>
...
export function MonthViewClient({
  ...
  const days = useMemo(
    () => buildMonthDays(month, blocks, exceptions),
  ...
  const monthBlocks = blocks.filter(
    (block) => dayjs(block.block_start).format("YYYY-MM") === monthKey
  );
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              ...
              >
                ...
                <div className="text-xs font-medium text-slate-500">
                  {formatHours(userMinutes)} im Monat
                </div>
              </div>
            );
          })}
      ...
      <div className="grid gap-4 xl:hidden">
        {days.map((day) => (
          <MobileDayCard key={day.key} day={day} month={month} />
        ))}
      </div>
      ...
      <section className="hidden rounded-[30px] border bg-white p-4 shadow-sm xl:block xl:p-6">
        <div className="grid grid-cols-6 gap-3">
          ...
          {days.map((day) => {
            const stats = getDayStats(day.blocks);
            const inMonth =
              day.date.format("YYYY-MM") === dayjs(month).format("YYYY-MM");
            const styles = getExceptionStyle(day.exception);
            ...
            return (
              <Link
                key={day.key}
                href={`/tagesansicht?date=${day.key}`}
                className={`min-h-[240px] rounded-[24px] border bg-white p-4 transition hover:shadow-md ${
                  inMonth ? "" : "opacity-50"
                } ${styles.dayClass}`}
              >
                <div className="mb-3 flex items-start justify-between gap-3">
                ...
                {stats.urgentCount > 0 ? (
                  <div className="mt-3 inline-flex items-center rounded-full bg-amber-50 px-2.5 py-1 text-[11px] font-medium text-amber-700">
